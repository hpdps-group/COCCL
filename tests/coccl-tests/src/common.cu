...
#if 1 && DEBUG_PRINT
    if (args->reportErrors && wrongPerGpu[i] != 0 && !in_place) {
      ...
      char *sendData = (char*)malloc(args->expectedBytes);
      ...
      cudaMemcpy(sendData, args->sendbuffs[i], args->expectedBytes, cudaMemcpyDeviceToHost);
      ...
      for(int j=0; j<args->expectedBytes/eltsz; j++) {
        ...
        send =0;
        memcpy(&send, sendData + j*eltsz, eltsz);

        if(want != got) {
          // printf(" rank=%d elt[%d]: want=0x%llx got=0x%llx\n", rank, j, want, got);
          printf(" rank=%d elt[%d]: send=%f got=%f\n", rank, j, send, got);
        }
      }
      free(expectedHost);
      free(dataHost);
