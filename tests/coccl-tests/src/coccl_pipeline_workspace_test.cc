...
void checkQueryShapes() {
  ...
  const cocclSizeQueryObservation& compress =
      cocclCompressQueryObservation();
  ...
  cocclConfigureSizeQueryStub(1, 4, 1, 2, true);
  context = {};
  EXPECT(cocclPreparePipeline(&allReduce, 4, &context) == ncclSuccess);
  const cocclSizeQueryObservation& drc =
      cocclDrcQueryObservation();
...
void dumpPlans() {
  ...
  cocclConfigureSizeQueryStub(1, 4, 1, 4, true);
...
int main(int argc, char** argv) {
  ...
  checkQueryShapes();
  std::printf("coccl workspace: PASS\n");
  return 0;
}
