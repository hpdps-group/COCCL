...
int main(int argc, char** argv) {
  ...
  std::printf("COCCL_TRAINING_COMM world_rank=%d kind=dp comm=%p expected=DP\n",
              worldRank, dp);
  std::printf("COCCL_TRAINING_COMM world_rank=%d kind=tp comm=%p expected=TP\n",
              worldRank, tp);
  std::printf("COCCL_TRAINING_COMM world_rank=%d kind=pp comm=%p expected=PP\n",
              worldRank, pp);
  std::fflush(stdout);
  ...
  std::printf("COCCL_TRAINING_PHASE world_rank=%d phase=observed_10\n", worldRank);
  std::fflush(stdout);
  ...
  TEST_CUDACHECK(cudaDeviceSynchronize());
  std::printf("COCCL_TRAINING_PHASE world_rank=%d phase=classification_expected\n",
              worldRank);
  std::fflush(stdout);
  ...
  std::printf(
      "COCCL_TRAINING_COMM world_rank=%d kind=ambiguous comm=%p expected=Unknown\n",
      worldRank, ambiguous);
  std::fflush(stdout);
  for (int iteration = 0; iteration < 10; ++iteration) {
  ...
  TEST_CUDACHECK(cudaDeviceSynchronize());
  std::printf("COCCL_TRAINING_PHASE world_rank=%d phase=unknown_expected\n",
              worldRank);
  std::fflush(stdout);
  ...
  TEST_CUDACHECK(cudaDeviceSynchronize());
  std::printf("COCCL_TRAINING_PHASE world_rank=%d phase=stable_complete\n",
              worldRank);
  std::fflush(stdout);
