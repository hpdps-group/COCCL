...
source_root=${1:?usage: m12_matrix.sh SOURCE_ROOT sdp4bit|zfp smoke|performance|memory}
compressor=${2:?usage: m12_matrix.sh SOURCE_ROOT sdp4bit|zfp smoke|performance|memory}
mode=${3:-performance}
...
timeout_seconds=${M12_CASE_TIMEOUT:-3600}
...
run_case() {
  ...
  if [[ -n ${M12_ONLY_STEM:-} && "$stem" != "$M12_ONLY_STEM" ]]; then
    return
  fi
  if [[ -f "$marker" && ${M12_FORCE:-0} != 1 ]]; then
    return
  fi
  [[ ${M12_FORCE:-0} != 1 ]] || rm -f "$marker"
...
case "$mode" in
  smoke)
    ...
    ;;
  performance|memory)
    run_matrix allgather pipeline
    run_matrix reducescatter oneshot
    run_matrix allreduce twoshot
    ;;
esac
