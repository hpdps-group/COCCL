...
source_root=${1:?usage: m11_build.sh SOURCE_ROOT}
cuda_root=${M11_CUDA_HOME:-/data/apps/cuda/12.4}
...
test_build=${M11_TESTS_DIR:-/data/home/scyb672/run/lxc/COCCL-migrate/tests/coccl-tests/build}
host_test_build=${M11_HOST_TEST_DIR:-/data/home/scyb672/run/codex-tmp/coccl-m11-host}
...
objects=(
  "$build_root/obj/misc/coccl_compressor_size.o"
...
"$host_test_build/coccl_m11_workspace_test" --csv \
  >"$result_root/layout_choice.csv"
{
  printf 'M11 encoded-size planning build manifest\n'
  printf 'NVCC_GENCODE=%s\n' "$gencode"
  printf 'host_objects=%s\n' "${objects[*]}"
  printf 'plugins=existing ABI v8 SDP4Bit TahQuant TACO ZFP\n'
  printf 'host_tests=M1-M11 PASS\n'
