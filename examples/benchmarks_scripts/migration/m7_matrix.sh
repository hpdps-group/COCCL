...
source_root=${1:?usage: m7_matrix.sh SOURCE_ROOT smoke|performance|layout|memory}
mode=${2:-performance}
...
timeout_seconds=${M7_CASE_TIMEOUT:-3600}
...
run_case() {
  ...
  local depth=$2
  local bytes=$3
  local executable=reduce_scatter_comp_oneshot_perf
  local algorithm=oneshot
  if [[ "$compressor" == native ]]; then
    executable=reduce_scatter_perf
    algorithm=native
  fi
  local stem="reducescatter__${algorithm}__${compressor}__d${depth}__b${bytes}"
  ...
  if [[ -n ${M7_ONLY_STEM:-} && "$stem" != "$M7_ONLY_STEM" ]]; then
    return
  fi
  if [[ -f "$marker" && ${M7_FORCE:-0} != 1 ]]; then
    return
  fi
