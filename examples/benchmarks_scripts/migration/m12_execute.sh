...
source_root=${M12_SOURCE_ROOT:-/data/home/scyb672/run/codex-tmp/coccl-valid-publish-20260816}
...
host_test_dir=${M12_HOST_TEST_DIR:-/data/home/scyb672/run/codex-tmp/coccl-m12-host}
tests=${M12_TESTS_DIR:-/data/home/scyb672/run/lxc/COCCL-migrate/tests/coccl-tests/build}
script_root="$source_root/examples/benchmarks_scripts/migration"
status_file="$result_root/execution.status"

export CUDA_HOME=${M12_CUDA_HOME:-/data/apps/cuda/12.4}
export PATH="$CUDA_HOME/bin:$PATH"
export LD_LIBRARY_PATH="$build_root/lib:$CUDA_HOME/lib64:${LD_LIBRARY_PATH:-}"

mkdir -p "$result_root"
"$host_test_dir/coccl_m12_inplace_plan_test" >"$result_root/host-inplace-plan.txt"
...
for compressor in sdp4bit zfp; do
  ...
  bash "$script_root/m12_matrix.sh" "$source_root" "$compressor" smoke
  printf '%s smoke-%s-complete\n' "$(date -Is)" "$compressor" >>"$status_file"
done
for compressor in sdp4bit zfp; do
  printf '%s performance-%s-start\n' "$(date -Is)" "$compressor" >>"$status_file"
  bash "$script_root/m12_matrix.sh" "$source_root" "$compressor" performance
  printf '%s performance-%s-complete\n' "$(date -Is)" "$compressor" >>"$status_file"
done
for compressor in sdp4bit zfp; do
  printf '%s memory-%s-start\n' "$(date -Is)" "$compressor" >>"$status_file"
  bash "$script_root/m12_matrix.sh" "$source_root" "$compressor" memory
  printf '%s memory-%s-complete\n' "$(date -Is)" "$compressor" >>"$status_file"
done

python3 "$script_root/m12_report.py" "$result_root" \
