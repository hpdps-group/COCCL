...
source_root=${M8_SOURCE_ROOT:-/data/home/scyb672/run/lxc/COCCL-migrate}
result_root=${M8_RESULT_ROOT:-/data/home/scyb672/run/lxc/COCCL-migrate/results/M8}
build_root=${M8_BUILD_ROOT:-$source_root/build}
host_test_dir=${M8_HOST_TEST_DIR:-/data/home/scyb672/run/codex-tmp/coccl-m8-host}
...
mkdir -p "$result_root"
"$host_test_dir/coccl_m8_allreduce_plan_test" >"$result_root/host-plan.txt"
"$host_test_dir/coccl_m8_allreduce_recipe_test" >"$result_root/host-recipe.txt"
"$host_test_dir/coccl_m8_allreduce_plan_test" --csv \
...
for mode in smoke performance layout memory; do
  ...
  bash "$script_root/m8_matrix.sh" "$source_root" "$mode"
  printf '%s %s-complete\n' "$(date -Is)" "$mode" >>"$status_file"
done

python3 "$script_root/m8_report.py" "$result_root" \
