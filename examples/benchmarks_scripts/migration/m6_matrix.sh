...
source_root=${1:?usage: m6_matrix.sh SOURCE_ROOT smoke|performance|layout|memory}
mode=${2:-performance}
...
timeout_seconds=${M6_CASE_TIMEOUT:-3600}
...
run_case() {
  ...
  local depth=$2
  local bytes=$3
  local executable=all_gather_comp_perf
  local algorithm=pipeline
  if [[ "$compressor" == native ]]; then
    executable=all_gather_perf
    algorithm=native
  fi
  local stem="allgather__${algorithm}__${compressor}__d${depth}__b${bytes}"
  ...
  if [[ -n ${M6_ONLY_STEM:-} && "$stem" != "$M6_ONLY_STEM" ]]; then
    return
  fi
  if [[ -f "$marker" && ${M6_FORCE:-0} != 1 ]]; then
    return
  fi
