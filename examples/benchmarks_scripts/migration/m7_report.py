...
def write_reducescatter(result_root):
    raw = result_root / "raw" / "performance"
    rows = []
    for marker in sorted(raw.glob("reducescatter__*.ok")):
    ...
    with (result_root / "reducescatter.csv").open("w", newline="") as stream:
        writer = csv.DictWriter(stream, fieldnames=fields)
        writer.writeheader()
        writer.writerows(rows)
    return rows, len(list(raw.glob("reducescatter__*.ok")))
...
def baseline_medians(baseline_root):
    ...
    with (baseline_root / "summary.csv").open() as stream:
        for row in csv.DictReader(stream):
            if row["variant"] != "original" or row["collective"] != "reducescatter":
                continue
            key = (
...
def compare_performance(result_root, rows, layout_rows, baseline_root):
    ...
    fields = [
        "algorithm", "compressor", "depth", "bytes", "inplace",
        "m0_us", "m7_us", "layout_us", "limit_us", "relative_delta",
        "gate", "rule", "passed",
    ]
...
def main():
    ...
    reducescatter_rows, performance_processes = write_reducescatter(result_root)
    layout_rows, layout_processes = write_layout(result_root)
    comparisons, gate_failures = compare_performance(
        result_root, reducescatter_rows, layout_rows, baseline_root
    )
    ...
    complete = (
        ...
        and len(reducescatter_rows) == EXPECTED_PERFORMANCE * 2
    ...
    with (result_root / "report.md").open("w") as stream:
        stream.write("# M7 Pipeline ReduceScatter Report\n\n")
        stream.write(f"Status: {'PASS' if passed else 'FAIL'}\n\n")
        stream.write("## Scope\n\n")
        stream.write("- OneShot path: Pack -> Compress -> AllToAll -> DecompressReduce for depth > 1.\n")
        stream.write("- TwoShot is compile/Host-only on this single-node milestone.\n")
        ...
        stream.write(f"- Performance processes: {performance_processes}/{EXPECTED_PERFORMANCE}; rows: {len(reducescatter_rows)}/{EXPECTED_PERFORMANCE * 2}.\n")
        ...
        stream.write("- `T_layout` follows the M5 definition: Pack + Unpack.\n\n")
        stream.write("## Workspace And Lifetime\n\n")
        stream.write("- OneShot depth 1 plans two R*C temps; depth 2/4/8 add R*C input staging per slice.\n")
        ...
        stream.write("## Notes\n\n")
        stream.write("- ReduceScatter numerical correctness remains outside this migration gate; benchmark completion and guard checks are recorded.\n")
        stream.write("- In-place columns are informational; overlapping input/output ranges use the serial fallback.\n")
    if not passed:
        raise SystemExit(1)
