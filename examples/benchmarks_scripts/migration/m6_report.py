...
def write_allgather(result_root):
    raw = result_root / "raw" / "performance"
    rows = []
    for marker in sorted(raw.glob("allgather__*.ok")):
    ...
    with (result_root / "allgather.csv").open("w", newline="") as stream:
        writer = csv.DictWriter(stream, fieldnames=fields)
        writer.writeheader()
        writer.writerows(rows)
    return rows, len(list(raw.glob("allgather__*.ok")))
...
def baseline_medians(baseline_root):
    ...
    with (baseline_root / "summary.csv").open() as stream:
        for row in csv.DictReader(stream):
            if row["variant"] != "original" or row["collective"] != "allgather":
                continue
            key = (
...
def compare_performance(result_root, rows, layout_rows, baseline_root):
    ...
    fields = [
        "algorithm", "compressor", "depth", "bytes", "inplace",
        "m0_us", "m6_us", "layout_us", "limit_us", "relative_delta",
        "gate", "rule", "passed",
    ]
...
def main():
    ...
    allgather_rows, performance_processes = write_allgather(result_root)
    layout_rows, layout_processes = write_layout(result_root)
    comparisons, gate_failures = compare_performance(
        result_root, allgather_rows, layout_rows, baseline_root
    )
    ...
    complete = (
        ...
        and len(allgather_rows) == EXPECTED_PERFORMANCE * 2
    ...
    with (result_root / "report.md").open("w") as stream:
        stream.write("# M6 Pipeline AllGather Report\n\n")
        stream.write(f"Status: {'PASS' if passed else 'FAIL'}\n\n")
        stream.write("## Scope\n\n")
        stream.write("- Pipeline path: Compress -> AllGather -> Decompress, with Unpack for depth > 1.\n")
        ...
        stream.write(f"- Performance processes: {performance_processes}/{EXPECTED_PERFORMANCE}; rows: {len(allgather_rows)}/{EXPECTED_PERFORMANCE * 2}.\n")
        ...
        stream.write("- Depth 1 plans C and R*C temps; depth 2/4/8 add an independent R*C output staging temp per slice.\n")
        ...
        stream.write("## Notes\n\n")
        stream.write("- AllGather numerical correctness remains outside this migration gate; benchmark completion and guard checks are recorded.\n")
        stream.write("- In-place columns are informational; overlapping input/output ranges fall back to depth 1 until the dedicated in-place milestone.\n")
    if not passed:
        raise SystemExit(1)
