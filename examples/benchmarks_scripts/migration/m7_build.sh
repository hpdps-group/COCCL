...
source_root=${1:?usage: m7_build.sh SOURCE_ROOT}
cuda_root=${CUDA_HOME:-/data/apps/cuda/12.4}
gencode=${NVCC_GENCODE:-'-gencode=arch=compute_80,code=sm_80'}
build_root=${M7_BUILD_ROOT:-$source_root/build}
test_build=${M7_TESTS_DIR:-$source_root/tests/coccl-tests/build}
host_test_build=${M7_HOST_TEST_DIR:-/data/home/scyb672/run/codex-tmp/coccl-m7-host}
zfp_objdir=${M7_ZFP_OBJDIR:-/data/home/scyb672/run/codex-tmp/coccl-m7-zfp}
...
make -C "$source_root/tests/coccl-tests/src" -j4 \
  ...
  "$test_build/reduce_scatter_perf" \
  "$test_build/reduce_scatter_comp_oneshot_perf" \
  "$test_build/reduce_scatter_comp_twoshot_perf" \
...
nm -D --defined-only "$lib_target" |
  grep -E ' pcocclReduceScatterCompOneShot$' >/dev/null
nm -D --defined-only "$lib_target" |
  grep -E ' pcocclReduceScatterCompTwoShot$' >/dev/null
if nm -D --defined-only "$lib_target" |
    grep -E ' p?ncclReduceScatterComp(OneShotOverlap|TwoShotOverlap|TwoShotTLOverlap)$' >/dev/null; then
  echo "retired ReduceScatter overlap symbol is still exported" >&2
  exit 1
fi
if grep -En 'ncclReduceScatterComp(OneShotOverlap|TwoShotOverlap|TwoShotTLOverlap)' \
    "$source_root/src/misc/nccl_comp_wrapper.cc" \
    "$source_root/src/nccl.h.in" >/dev/null; then
  echo "retired ReduceScatter overlap source is still present" >&2
  exit 1
fi
