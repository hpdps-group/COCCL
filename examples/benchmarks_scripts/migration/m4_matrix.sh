...
source_root=${1:?usage: m4_matrix.sh SOURCE_ROOT smoke|performance|memory|lifecycle}
mode=${2:-performance}
result_root=${M4_RESULT_ROOT:-/data/home/scyb672/run/lxc/COCCL-migrate/results/M4}
build_root=${M4_BUILD_ROOT:-$source_root/build}
tests=${M4_TESTS_DIR:-$source_root/tests/coccl-tests/build}
...
timeout_seconds=${M4_CASE_TIMEOUT:-3600}
...
set_case_environment() {
  ...
  [[ "$backend" == vmm ]] || export NCCL_CUMEM_ENABLE=0
...
run_case() {
  ...
  local stem="alltoall__${algorithm}__${compressor}__${backend}__b${bytes}"
  ...
  if [[ -n ${M4_ONLY_STEM:-} && "$stem" != "$M4_ONLY_STEM" ]]; then
    return
  fi

  if [[ -f "$marker" && ${M4_FORCE:-0} != 1 ]]; then
    return
  fi

  set_case_environment "$backend" "$compressor"
  export NCCL_DEBUG=WARN
  unset NCCL_DEBUG_SUBSYS
  if [[ "$mode" == smoke || "$mode" == lifecycle ]]; then
    export NCCL_DEBUG=INFO
    export NCCL_DEBUG_SUBSYS=INIT
  fi

  {
    printf 'COCCL_ENABLE=%q NCCL_ENABLE_COMPRESS=0 NCCL_CUMEM_ENABLE=%q NCCL_DEBUG=%q ' \
      "$COCCL_ENABLE" "$NCCL_CUMEM_ENABLE" "$NCCL_DEBUG"
    if [[ "$compressor" != native ]]; then
...
run_size() {
  local backend=$1
  local bytes=$2
  run_case "$backend" native alltoall_p2p_perf "$bytes"
  run_case "$backend" sdp4bit alltoall_comp_overlap_perf "$bytes"
  run_case "$backend" zfp alltoall_comp_overlap_perf "$bytes"
}

case "$mode" in
  smoke)
    run_size legacy 67108864
    run_size vmm 67108864
    ;;
  performance)
  ...
  memory)
    ...
    ;;
  lifecycle)
    run_case vmm sdp4bit alltoall_comp_overlap_perf 67108864
    ;;
  *)
