...
source_root=${1:?usage: m6_build.sh SOURCE_ROOT}
cuda_root=${CUDA_HOME:-/data/apps/cuda/12.4}
gencode=${NVCC_GENCODE:-'-gencode=arch=compute_80,code=sm_80'}
build_root=${M6_BUILD_ROOT:-$source_root/build}
test_build=${M6_TESTS_DIR:-$source_root/tests/coccl-tests/build}
host_test_build=${M6_HOST_TEST_DIR:-/data/home/scyb672/run/codex-tmp/coccl-m6-host}
zfp_objdir=${M6_ZFP_OBJDIR:-/data/home/scyb672/run/codex-tmp/coccl-m6-zfp}
...
make -C "$source_root/tests/coccl-tests/src" -j4 \
  ...
  "$test_build/all_gather_perf" \
  "$test_build/all_gather_comp_perf" \
  "$test_build/coccl_m5_pipeline_layout_test"

for target in m1-host-tests m1-plugin-load-test m2-host-tests \
              m3-host-tests m4-host-tests m5-host-tests m6-host-tests; do
  make -C "$source_root/tests/coccl-tests/src" "$target" \
    BUILDDIR="$host_test_build" CUDA_HOME="$CUDA_HOME" NCCL_HOME="$NCCL_HOME" \
    DEBUG=0
done

nm -D --defined-only "$lib_target" | grep -E ' pcocclAllGatherComp$' >/dev/null
if nm -D --defined-only "$lib_target" |
    grep -E ' p?ncclAllGatherCompOverlap$' >/dev/null; then
  echo "retired AllGather overlap symbol is still exported" >&2
  exit 1
fi
if grep -En 'rankSlot|cudaMemcpyAsync' \
    "$source_root/src/misc/coccl_pipeline_stage.cc" \
    >/dev/null; then
  echo "AllGather stage contains an in-place rank-slot copy" >&2
  exit 1
fi
