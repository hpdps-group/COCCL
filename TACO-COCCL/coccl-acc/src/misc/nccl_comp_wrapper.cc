...
// extern bool enableCheck;

// ncclResult_t cocclCheck(const void* sendbuff, const void* recvbuff, size_t chunkcount, ncclDataType_t datatype, ncclRedOp_t redOp, ncclCommOp_t commOp, ncclComm_t comm){
//   char *ss;
//   size_t numchunks = 1;
//   void* expect_recvbuff = nullptr;

//   switch(commOp){
//     case ncclCommOp_t::AlltoAll:
//     case ncclCommOp_t::AlltoAll_Inter:
//       ss="AlltoAll";
//       numchunks=comm->nRanks;
//       CUDACHECK(cudaMalloc((void**)&expect_recvbuff, chunkcount * numchunks * ncclTypeSize(datatype)));
//       NCCLCHECK(ncclAllToAll((void*)sendbuff, (void*)expect_recvbuff, chunkcount, datatype, comm, 0));
//       break;
...
//       numchunks=comm->nRanks;
//       CUDACHECK(cudaMalloc((void**)&expect_recvbuff, chunkcount * numchunks * ncclTypeSize(datatype)));
//       NCCLCHECK(ncclAllReduce((void*)sendbuff, (void*)expect_recvbuff, chunkcount * numchunks, datatype, redOp, comm, 0));
//       break;
...
//       // printf("dasdasdsadas\n");
//       CUDACHECK(cudaMalloc((void**)&expect_recvbuff, chunkcount * numchunks * ncclTypeSize(datatype)));
//       NCCLCHECK(ncclAllGather(sendbuff, expect_recvbuff, chunkcount, datatype, comm, 0));
//       break;
...
//       numchunks=1;
//       CUDACHECK(cudaMalloc((void**)&expect_recvbuff, chunkcount * numchunks * ncclTypeSize(datatype)));
//       NCCLCHECK(ncclReduceScatter((void*)sendbuff, (void*)expect_recvbuff, chunkcount, datatype, redOp, comm, 0));
//       break;
//     case ncclCommOp_t::SendRecv:
//     case ncclCommOp_t::SendRecv_BWD:
//       ss="SendRecv";
//       numchunks=1;
//       CUDACHECK(cudaMalloc((void**)&expect_recvbuff, chunkcount * numchunks * ncclTypeSize(datatype)));
//       CUDACHECK(cudaMemset(expect_recvbuff, 0, chunkcount * numchunks * ncclTypeSize(datatype)));
//       break;
//     default:
//       ss="";
//   }
...
//     float *recv = (float*) malloc(chunkcount * numchunks * ncclTypeSize(datatype));
//     float *expect_recv = (float*) malloc(chunkcount * numchunks * ncclTypeSize(datatype));
//     CUDACHECK(cudaMemcpy((void*)recv, recvbuff, chunkcount * numchunks * ncclTypeSize(datatype), cudaMemcpyDeviceToHost));
//     CUDACHECK(cudaMemcpy((void*)expect_recv, expect_recvbuff, chunkcount * numchunks * ncclTypeSize(datatype), cudaMemcpyDeviceToHost));

//     CUDACHECK(cudaDeviceSynchronize());  
//     float delta = 0;
//     for(size_t i = 0; i< chunkcount * numchunks; i++){
//       // INFO(NCCL_INIT, "%s_rank_%d_chunkIdx_%lu_elemIdx_%lu_expect_%f_recv_%f",ss, comm->rank, i/chunkcount, i%chunkcount, expect_recv[i], recv[i]);
//       delta += std::fabs(expect_recv[i]-recv[i]) / (expect_recv[i] + 0.000001);
//     }
//     INFO(NCCL_INIT, "%s_rank_%d_error_%f",ss, comm->rank, delta / chunkcount / numchunks);
//     free(recv);
//     free(expect_recv);
//   }
//   CUDACHECK(cudaDeviceSynchronize());  
//   CUDACHECK(cudaFree(expect_recvbuff));  
//   return ncclSuccess;
// }
#include "nccl_comp_wrapper.h"
#include "nccl.h"
...
static void dump_bin(
  ...
  snprintf(
    filename,
    sizeof(filename),
    "/data/home/scyb226/logs/coccl_%s_rank%d_dtype%d_%s.bin",
    op, rank, (int)datatype, tag
  );

  FILE* fp = fopen(filename, "ab");  // append
  if (!fp) return;

  fwrite(buf, ncclTypeSize(datatype), elem_count, fp);
  fclose(fp);
}

/* ---------------- helper: dump relative error (FP32 only) ---------------- */
static void dump_error_fp32(
    const char* op,
    int rank,
    const float* expect,
    const float* recv,
  ...
  snprintf(
    filename,
    sizeof(filename),
    "/data/home/scyb226/lm/coccl-training/logs/350M/coccllogs/coccl_%s_rank%d_error.bin",
    op, rank
  );
  ...
  for (size_t i = 0; i < elem_count; i++) {
    float err = fabs(expect[i] - recv[i]) /
                (fabs(expect[i]) + 1e-6f);
    fwrite(&err, sizeof(float), 1, fp);
  }
  fclose(fp);
}

/* ========================= cocclCheck ========================= */
ncclResult_t cocclCheck(
    const void* sendbuff,
    const void* recvbuff,
    size_t chunkcount,
    ncclDataType_t datatype,
  ...
  /* -------- generate golden result using NCCL -------- */
  switch (commOp) {
    case ncclCommOp_t::AlltoAll:
    case ncclCommOp_t::AlltoAll_Inter:
      op_name = "AlltoAll";
      ...
      CUDACHECK(cudaMalloc(
          &expect_recvbuff,
          chunkcount * numchunks * ncclTypeSize(datatype)));
    ...
    case ncclCommOp_t::AllReduce_Inter:
      ...
      CUDACHECK(cudaMalloc(
          &expect_recvbuff,
          chunkcount * ncclTypeSize(datatype)));
  ...
  /* -------- compare & dump (rank 0 only) -------- */
  if (sendbuff != recvbuff && comm->rank == 0) {
    CUDACHECK(cudaDeviceSynchronize());

    size_t elem_cnt = chunkcount * numchunks;
    size_t bytes = elem_cnt * ncclTypeSize(datatype);

    void* recv = malloc(bytes);
    void* expect_recv = malloc(bytes);
    ...
    // dump expect & recv
    dump_bin(op_name, "expect", comm->rank, datatype,
             expect_recv, elem_cnt);
    dump_bin(op_name, "recv", comm->rank, datatype,
             recv, elem_cnt);

    // compute & dump error (FP32 only)
    ...
    if (datatype == ncclFloat32) {
      float delta = 0.f;
      float* e = (float*)expect_recv;
      float* r = (float*)recv;
      for (size_t i = 0; i < elem_cnt; i++) {
        delta += fabs(e[i] - r[i]) / (fabs(e[i]) + 1e-6f);
      }
      INFO(NCCL_INIT,
           "%s_rank_%d_mean_rel_error_%e",
           op_name, comm->rank,
           delta / elem_cnt);
    }
...
ncclResult_t ncclAllGatherComp(const void* sendbuff, void* recvbuff, size_t sendcount,
  ...
  if(enableCheck) cocclCheck(sendbuff, recvbuff, sendcount, datatype, ncclRedOp_t::ncclSum, ncclCommOp_t::AllGather, comm);
...
ncclResult_t  ncclAllToAllComp(const void* sendbuff, void* recvbuff, size_t sendcount,
  ...
  NCCLCHECK(ncclCompress(sendbuff, &a2Abuff, sendcount, datatype, &compSendCount, &compDatatype, comm->nRanks, ncclCommOp_t::AlltoAll, stream));
  ...
  if(enableCheck) cocclCheck(sendbuff, recvbuff, sendcount, datatype, ncclRedOp_t::ncclSum, ncclCommOp_t::AlltoAll, comm);
...
ncclResult_t ncclReduceScatterCompOneShot(const void* sendbuff, void* recvbuff, size_t recvcount,
  ...
  if(enableCheck) cocclCheck(sendbuff, recvbuff, recvcount, datatype, ncclRedOp_t::ncclSum, ncclCommOp_t::ReduceScatter, comm);
  return ncclSuccess;
}
...
ncclResult_t ncclReduceScatterComp(const void* sendbuff, void* recvbuff, size_t recvcount,
  ...
  if(enableCheck) cocclCheck(sendbuff, recvbuff, recvcount, datatype, op, ncclCommOp_t::ReduceScatter, comm);
  return ncclSuccess;
}
...
ncclResult_t ncclReduceScatterCompTwoShotTLOverlap(const void* sendbuff, void* recvbuff, size_t recvcount,
  ...
  for(size_t i =0 ;i<pipelineDepth; i++){
    ...
    NCCLCHECK(ncclDecompReduceComp((void*)intraRecvCompbuff, &interSendCompbuff, compSendCount * nNodes, datatype,
                ...
                RSdecompStream));
    CUDACHECK(cudaEventRecord(RSdecompEvent, RSdecompStream));  
    CUDACHECK(cudaStreamWaitEvent(RScommStream_Inter, RSdecompEvent, 0));
    ...
    NCCLCHECK(ncclAllToAll((void*)interSendCompbuff, (void*)interRecvCompbuff, interSendCount, reCompDatatype, InterSubComm, RScommStream_Inter));
    CUDACHECK(cudaEventRecord(RScommEvent_Inter, RScommStream_Inter));  
    CUDACHECK(cudaStreamWaitEvent(RSdecompStream_Inter, RScommEvent_Inter, 0));
    void* piperecvbuff = (char*) recvbuff + i * recvcount / pipelineDepth * ncclTypeSize(datatype);

    // DecompReduce
    NCCLCHECK(ncclDecompressReduce((void*)piperecvbuff, interRecvCompbuff, interSendCount, reCompDatatype, recvcount / pipelineDepth, datatype, nNodes,
                          ncclCommOp_t::ReduceScatter_Inter, RSdecompStream_Inter));
  }
  CUDACHECK(cudaEventRecord(RSdecompEvent_Inter, RSdecompStream_Inter));
...
ncclResult_t ncclAllReduceCompOneShot(const void* sendbuff, void* recvbuff, size_t count,
  ...
  if(enableCheck) cocclCheck(sendbuff, recvbuff, count / comm->nRanks, datatype, op, ncclCommOp_t::AllReduce, comm);
...
ncclResult_t ncclRecvDecomp(void* recvbuff, size_t count, ncclDataType_t datatype, int peer,
  ...
  if(enableCheck) cocclCheck(sendbuff, recvbuff, count, datatype, ncclRedOp_t::ncclSum, compop, comm);
