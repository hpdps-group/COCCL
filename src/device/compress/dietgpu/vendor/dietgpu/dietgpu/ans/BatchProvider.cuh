...
struct BatchProviderStride {
  ...
  __device__ bool isActive(uint32_t) const {
    return true;
  }
...
struct BatchProviderStrideMasked : BatchProviderStride {
  __host__ BatchProviderStrideMasked(
      void* ptr_dev,
      uint32_t batchStride,
      const uint8_t* active_dev,
      uint32_t batchCapacity = 0)
      : BatchProviderStride(ptr_dev, batchStride, batchCapacity),
        active_dev_(active_dev) {}

  __device__ bool isActive(uint32_t batch) const {
    return active_dev_[batch] != 0;
  }

  const uint8_t* active_dev_;
};
