...
// COCCL extension: inactive batch members are ignored by the ANS kernels.
// Their output can be populated by a separate raw-frame copy kernel.
ANSDecodeStatus ansDecodeBatchStrideMasked(
    StackDeviceMemory& res,
    const ANSCodecConfig& config,
    uint32_t numInBatch,
    const void* in_dev,
