...
template <int q_bits,
          ...
          int max_threads>
__global__ void cached_quantization_ht(int8_t* __restrict__ output_data,
...
#pragma unroll
    ...
    /**** Start 32*32 Hadamard Transform ****/
    /* Hadamard Transform Function Refer to https://github.com/Dao-AILab/fast-hadamard-transform/blob/master/csrc/fast_hadamard_transform_cuda.cu */
...
#pragma unroll
    for (int i = 0; i < UNROLL; i++) {
        __nv_bfloat162* iteration_buffer = local_buffer + i * internal_unroll * quantize::h2_per_load;
...
/********* Launcher methods ***********/
#define LAUNCH_CACHED_QUANT_HT_CALL(q_bits, quant_type) \
    cached_quantization_ht<q_bits,                      \
                        quant_type,                  \
                        unroll_factor,               \
                        internal_unroll_l,           \
                        threads_per_group,           \
...
#define LAUNCH_CACHED_QUANT_HT(                                                        \
    ...
    if (q_bits == 4) {                                                              \
        if (quant_type == quantize::Type::Asymmetric) {                             \
            LAUNCH_CACHED_QUANT_HT_CALL(4, quantize::Type::Asymmetric)                 \
        } else {                                                                    \
            LAUNCH_CACHED_QUANT_HT_CALL(4, quantize::Type::Symmetric)                  \
        }                                                                           \
    } else {                                                                        \
        if (quant_type == quantize::Type::Asymmetric) {                             \
            LAUNCH_CACHED_QUANT_HT_CALL(8, quantize::Type::Asymmetric)                 \
        } else {                                                                    \
            LAUNCH_CACHED_QUANT_HT_CALL(8, quantize::Type::Symmetric)                  \
        }                                                                           \
    }
                  ...
                  cudaStream_t stream)
{
    ...
    if (is_subblock_schedule) {
        ...
        if (threads_per_group == 1) {
            LAUNCH_CACHED_QUANT_HT(num_bits, quant_type, 1, 1, 1);
        } else if (threads_per_group == 2) {
            LAUNCH_CACHED_QUANT_HT(num_bits, quant_type, 1, 1, 2);
        } else if (threads_per_group == 4) {
            LAUNCH_CACHED_QUANT_HT(num_bits, quant_type, 1, 1, 4);
        } else if (threads_per_group == 8) {
            LAUNCH_CACHED_QUANT_HT(num_bits, quant_type, 1, 1, 8);
        } else if (threads_per_group == 16) {
            LAUNCH_CACHED_QUANT_HT(num_bits, quant_type, 1, 1, 16);
        }
    } else if (external_unroll == 1) {
        // 129 - 4096 elems
        // (this can launch with 1-7 warps as well)
        LAUNCH_CACHED_QUANT_HT(num_bits, quant_type, 1, internal_unroll, max_threads);
    } else if (external_unroll == 2) {
        // 4097 - 8192 elems
        LAUNCH_CACHED_QUANT_HT(num_bits, quant_type, 2, internal_unroll, max_threads);
    } else if (external_unroll == 3) {
        // 8193 - 12288 elems
        LAUNCH_CACHED_QUANT_HT(num_bits, quant_type, 3, internal_unroll, max_threads);
    } else if (external_unroll == 4) {
        // 12289 - 16384 elems
        LAUNCH_CACHED_QUANT_HT(num_bits, quant_type, 4, internal_unroll, max_threads);
    }
}
                  ...
                  cudaStream_t stream) 
{
    ...
    if (is_subblock_schedule) {
        ...
        if (threads_per_group == 1) {
            ...
            LAUNCH_CACHED_QUANT_HT(num_bits, quant_type, 1, 1, 32);
        }
    } else if (external_unroll == 1) {
        // 129 - 4096 elems
        // (this can launch with 1-7 warps as well)
