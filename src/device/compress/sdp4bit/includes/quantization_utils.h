...
constexpr int f_per_load = granularity / sizeof(float);
...
public:
    ...
    4) Asymmetric
    Function Arguments :
        val : The __half or float value to quantize.
    */
    template <typename T>
    DS_D_INLINE int8_t quantize(T val);

    template <typename T>
    DS_D_INLINE int8_t stochastic_quantize(T val);
...
public:
    ...
    DS_D_INLINE int8_t stochastic_quantize(T val)
    {
        ...
        float val_f = conversion::to<float>(val) * scale;
        float lower = floorf(val_f);
        float upper = ceilf(val_f);
        float prob = val_f - lower;
        curandState state;
        ...
        int32_t data_i32;
        if (randomValue < prob) {
            data_i32 = static_cast<int32_t>(upper);
        } else {
            data_i32 = static_cast<int32_t>(lower);
        }
        data_i32 = min(max(data_i32, q_min), q_max);
        return (int8_t)data_i32;
    }
...
public:
    ...
    DS_D_INLINE void store(float* params, int group_index)
    {
        ...
        mem_access::store_global<sizeof(float)>(params + group_index, &store_scale);
        mem_access::store_global<sizeof(float)>(params + group_index + 1, &offset);
    }
    ...
    DS_D_INLINE Params(const float* params, int group_index)
    {
        ...
        mem_access::load_global<sizeof(float)>(&scale, params + group_index);
        mem_access::load_global<sizeof(float)>(&offset, params + group_index + 1);
    }
};
...
*/
template <Type qType, typename T>
class GroupStats {
public:
    DS_D_INLINE void update(T val);
...
#ifdef BF16_AVAILABLE
template <>
class GroupStats<Type::Symmetric, __nv_bfloat162> {
public:
    // Symmetric quantization only tracks the maximum absolute value
    __nv_bfloat162 cur_max;
    float max;
    ...
    */
    DS_D_INLINE GroupStats() { cur_max = reduce::init<rop::Max, __nv_bfloat162>(); }
    ...
    */
    DS_D_INLINE void update(__nv_bfloat162 val)
    {
...
template <>
class GroupStats<Type::Symmetric, __half2> {
public:
    // Symmetric quantization only tracks the maximum absolute value
    __half2 cur_max;
    float max;
...
template <>
class GroupStats<Type::Symmetric, float> {
public:
    // Symmetric quantization only tracks the maximum absolute value
    float cur_max;
    float max;
    ...
    to be a reasonable edge case.
    */
    DS_D_INLINE GroupStats() { cur_max = reduce::init<rop::Max, float>(); }

    /*
    Updated the running absmax used to calculate params.
    Function Arguments :
        val : The float value to update the running min and max with.
    */
    DS_D_INLINE void update(float val)
    {
...
#ifdef BF16_AVAILABLE
template <>
class GroupStats<Type::Asymmetric, __nv_bfloat162> {
public:
    __nv_bfloat162 cur_max;
    __nv_bfloat162 cur_min;
    ...
    */
    ...
    {
        cur_max = reduce::init<rop::Max, __nv_bfloat162>();
        cur_min = reduce::init<rop::Min, __nv_bfloat162>();
    }

    /*
    Updated the running min and max used to calculate params.
    Function Arguments :
        val : The __nv_bfloat162 value to update the running min and max with.
    */
    DS_D_INLINE void update(__nv_bfloat162 val)
    {
        cur_max = reduce::element<rop::Max>(cur_max, val);
...
template <>
class GroupStats<Type::Asymmetric, __half2> {
public:
    __half2 cur_max;
    __half2 cur_min;
...
template <>
class GroupStats<Type::Asymmetric, float> {
public:
    float cur_max;
    float cur_min;
    ...
    */
    DS_D_INLINE GroupStats()
    {
        cur_max = reduce::init<rop::Max, float>();
        cur_min = reduce::init<rop::Min, float>();
    }
...
Function Arguments :
    local_output -  Pointer to local memory to store quantized data.    int8_t*
    data         -  Pointer to input data.                              float*
    Params       -  Parameters for quantization.                        Params<qType, numBits>
*/
template <int numBits, Type qType>
DS_D_INLINE void _chunk(int8_t* local_output, const float* data, Params<qType, numBits> q_params);
...
DS_D_INLINE GroupStats<qType, __half2> _local_serial_reduce(__half2* local_buffer);
...
Template Arguments :
    ...
    numChunks   -   Number of bits in quantized element.    int : 8 or 4
Function Arguments :
    local_buffer    -   Pointer memory with input float data to be quantized.
*/
template <Type qType, int numChunks>
DS_D_INLINE GroupStats<qType, float> _local_serial_reduce(float* local_buffer);
...
#pragma unroll
    for (int i = 0, oi = 0; i < elems; i += num_elems_packed, oi++) {
        if (num_elems_packed == 1) {
            ...
            local_output[i] = q_params.stochastic_quantize(data[i]);
        } else if (num_elems_packed == 2) {
            int8_t data_i8_1 = q_params.stochastic_quantize(data[i]);
...
#ifdef BF16_AVAILABLE
template <int numBits, Type qType>
DS_D_INLINE void _chunk(int8_t* local_output, const __nv_bfloat16* data, Params<qType, numBits> q_params)
{
    constexpr int32_t elems = 16 / sizeof(__nv_bfloat16);
    constexpr int32_t num_elems_packed = 8 / numBits;
...
DS_D_INLINE void _chunk(int8_t* local_output, const float* data, Params<qType, numBits> q_params)
{
    constexpr int32_t elems = 16 / sizeof(float);
    constexpr int32_t num_elems_packed = 8 / numBits;
...
#ifdef BF16_AVAILABLE
template <int numBits, Type qType>
DS_D_INLINE void _chunk(int8_t* local_output, const __nv_bfloat162* data, Params<qType, numBits> q_params)
{
    const __nv_bfloat16* data_cast = reinterpret_cast<const __nv_bfloat16*>(data);
    _chunk<numBits>(local_output, data_cast, q_params);
}


template <Type qType, int numChunks>
DS_D_INLINE GroupStats<qType, __nv_bfloat162> _local_serial_reduce(__nv_bfloat162* local_buffer)
{
    GroupStats<qType, __nv_bfloat162> stats;
#pragma unroll
    for (int i = 0; i < numChunks * bf2_per_load; i++) { stats.update(local_buffer[i]); }
...
DS_D_INLINE GroupStats<qType, __half2> _local_serial_reduce(__half2* local_buffer)
{
    GroupStats<qType, __half2> stats;
#pragma unroll
...
DS_D_INLINE GroupStats<qType, float> _local_serial_reduce(float* local_buffer)
{
    GroupStats<qType, float> stats;
#pragma unroll
    for (int i = 0; i < numChunks * f_per_load; i++) { stats.update(local_buffer[i]); }
                             ...
                             Params<qType, numBits> q_params)
{
    ...
    const int64_t elems_per_params = 2 * sizeof(float) * num_ele_int8;
    const int64_t block_offset = block_num * (elems_per_group + elems_per_params);
    ...
    const int64_t quant_offset = base_offset + 2 * sizeof(float);
    ...
    if (tb.thread_index().x == 0 && block_num < groups) {
        ...
        //     block_num);
        q_params.store(
            reinterpret_cast<float*>(output_data + base_offset),
            0);
    }
#pragma unroll
    for (int i = 0; i < numChunks; i++) {
        if (elem_offset + i * stride * num_ele_int8 < elems_per_group && block_num < groups) {
            quantize::_chunk<numBits, qType>(
                local_output, local_buffer + i * quantize::bf2_per_load, q_params);
            mem_access::store_global<num_int8_out>(output_data + (quant_offset + i * stride),
                                                   local_output);
        }
    }
}
                             ...
                             Params<qType, numBits> q_params)
{
    ...
    const int64_t block_num =
        (tb.group_index().x * max_threads / threads_per_group) + tb.thread_index().y;
    const int64_t params_size = (qType == Type::Asymmetric ? 2 : 1) * sizeof(float);
    const int64_t elems_per_params = params_size * num_ele_int8;
    const int64_t block_offset = block_num * (elems_per_group + elems_per_params);
    const int elem_offset = tb.thread_index().x * quantize::f_per_load;
    const int64_t base_offset = (block_offset + elem_offset) / num_ele_int8;
    const int64_t quant_offset = base_offset + params_size;
...
template <Type qType,
          ...
          int max_threads = 256>
__device__ void local_array(__nv_bfloat162* local_buffer,
                            float* __restrict__ global_params,
                            int8_t* __restrict__ output_data,
                            const int& elems_per_group,
                            const int& groups)
...
template <Type qType,
          ...
          int threads_per_group = max_threads,
          int max_threads = 256>
__device__ void local_array(float* local_buffer,
                            float* __restrict__ global_params,
                            int8_t* __restrict__ output_data,
                            const int& elems_per_group,
                            const int& groups)
