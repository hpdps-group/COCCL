...
__global__ void swizzled_quant_kernel_ht(int8_t* quantized_data,
                                      ...
                                      int devices_per_node)
{
    ...
    quantize::GroupStats<quantType, __half2> stats;
#pragma unroll
                                            ...
                                            int devices_per_node)
{
    ...
    // Local buffer
    float local_buffer[totalChunks * quantize::f_per_load];
...
#pragma unroll
    ...
    const int out_stride = stride / out_scalar_effect;
    constexpr int num_int8_out = quantize::f_per_load / out_scalar_effect;
...
#pragma unroll
    ...
    // start fixed 32*32 Hadamard Transform, accroding https://github.com/Dao-AILab/fast-hadamard-transform/blob/master/csrc/fast_hadamard_transform_cuda.cu
    constexpr int kNChunks = 1;
    constexpr int kNElts = quantize::bf_per_load;
    constexpr int kLogNElts = cilog2(kNElts);
    constexpr int kNWarps = 32 / kNElts;
    constexpr int kLogWarpSize = cilog2(kNWarps);
#pragma unroll
...
#define LAUNCH_SWIZZLE_QUANT_HT(total_chunks, threads)                                           \
    swizzled_quant_kernel_ht<numBits, total_chunks, threads, qType><<<grid, block, 0, stream>>>( \
...
void launch_swizzled_quant_impl_ht(int8_t* q_data,
                                float* q_scales,
                                ...
                                cudaStream_t stream)
{
    ...
    if (threads == 32) {
        LAUNCH_SWIZZLE_QUANT_HT(2, 32);
    } else if (threads == 64) {
        LAUNCH_SWIZZLE_QUANT_HT(2, 64);
    } else if (threads == 128) {
        LAUNCH_SWIZZLE_QUANT_HT(2, 128);
    } else if (threads == 256) {
        LAUNCH_SWIZZLE_QUANT_HT(2, 256);
    } else if (threads == 512) {
        if (total_unroll == 2) {
            LAUNCH_SWIZZLE_QUANT_HT(2, 512);
        } else if (total_unroll == 4) {
            LAUNCH_SWIZZLE_QUANT_HT(4, 512);
        } else if (total_unroll == 6) {
            LAUNCH_SWIZZLE_QUANT_HT(6, 512);
        } else if (total_unroll == 8) {
            LAUNCH_SWIZZLE_QUANT_HT(8, 512);
        } else if (total_unroll == 10) {
            LAUNCH_SWIZZLE_QUANT_HT(10, 512);
        }
    }
}

// #define LAUNCH_SWIZZLE_QUANT_FLOAT_HT(total_chunks, threads)                                           \
//     swizzled_quant_kernel_float_ht<numBits, total_chunks, threads, qType><<<grid, block, 0, stream>>>( \
...
{
    const int one_step_threads =
        next_pow2((elems_per_group + swiz_quant::f_per_step - 1) / swiz_quant::f_per_step);
    const int max_threads = (one_step_threads < swiz_quant::max_threads) ? one_step_threads
                                                                         : swiz_quant::max_threads;
    const int threads = (max_threads < swiz_quant::min_threads) ? swiz_quant::min_threads
                                                                : max_threads;
    ...
    const int elems_per_step = threads * swiz_quant::f_per_step;
    const int external_unroll = ((elems_per_group + elems_per_step - 1) / elems_per_step);
    const int total_unroll = external_unroll * swiz_quant::step_granularity;
...
#define DISPATCH_SWIZZLE_QUANT_HT(num_bits, qtype)                   \
    launch_swizzled_quant_impl_ht<num_bits, qtype>(q_data,           \
                                                q_scales,         \
                                                input_data,       \
                                                num_chunks,           \
                                                elems_per_chunk,  \
                           ...
                           cudaStream_t stream)
{
    if (num_bits == 4) {
        if (q_type == quantize::Type::Asymmetric) {
            DISPATCH_SWIZZLE_QUANT_HT(4, quantize::Type::Asymmetric);
        } else if (q_type == quantize::Type::Symmetric) {
            DISPATCH_SWIZZLE_QUANT_HT(4, quantize::Type::Symmetric);
        }
    } else if (num_bits == 8) {
        if (q_type == quantize::Type::Asymmetric) {
            DISPATCH_SWIZZLE_QUANT_HT(8, quantize::Type::Asymmetric);
        } else if (q_type == quantize::Type::Symmetric) {
            DISPATCH_SWIZZLE_QUANT_HT(8, quantize::Type::Symmetric);
        }
    }
}
