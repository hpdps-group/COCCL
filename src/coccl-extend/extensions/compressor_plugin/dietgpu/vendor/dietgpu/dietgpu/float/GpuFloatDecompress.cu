...
FloatDecompressStatus floatDecompressBatchStrideMasked(
    ...
    uint32_t* outSize_dev,
    cudaStream_t stream) {
  auto inProvider = BatchProviderStrideMasked(
      const_cast<void*>(in_dev), inPerBatchStride, active_dev);
  auto outProvider = BatchProviderStride(
      out_dev, outPerBatchStride, outPerBatchCapacity);
  return floatDecompressDevice(
      res,
      config,
      numInBatch,
