...
// COCCL extension for uniform, strided frames.
void floatCompressBatchStride(
    StackDeviceMemory& res,
    const FloatCompressConfig& config,
    uint32_t numInBatch,
    const void* in_dev,
...
// COCCL extension for uniform, strided frames. Inactive members are skipped
// so Raw and float-encoded frames can share one batch.
FloatDecompressStatus floatDecompressBatchStrideMasked(
    StackDeviceMemory& res,
    const FloatDecompressConfig& config,
    uint32_t numInBatch,
    const void* in_dev,
