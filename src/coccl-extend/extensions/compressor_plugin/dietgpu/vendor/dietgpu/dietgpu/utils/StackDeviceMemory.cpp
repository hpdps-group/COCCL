...
void* StackDeviceMemory::Stack::getAlloc(
  ...
  const bool needsOverflow =
      size > sizeRemaining || type == AllocType::Permanent;
  CHECK(!needsOverflow || allowOverflow_)
      << "externally supplied StackDeviceMemory is too small";
  if (needsOverflow) {
