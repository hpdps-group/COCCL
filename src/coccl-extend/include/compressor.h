...
// Plugin ABI implemented by external compressor shared objects. Runtime loads a
// symbol whose name matches the compressor entry in NCCL_COMPRESSORS and then
// calls through this vtable.
// A compressor can provide fused decomp-reduce or decomp-reduce-recompress
// callbacks; the runtime falls back to decompression plus COCCL reduction when
// those optional callbacks are null.
typedef struct {
    ...
    // Optional fused callback for reduce-scatter/allreduce receive paths.
    ...
    // Optional fused callback used when the next communication stage still
    // expects compressed data after reduction.
