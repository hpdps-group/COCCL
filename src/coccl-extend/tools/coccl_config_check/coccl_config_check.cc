...
int main(int argc, char** argv) {
  ...
  for (const cocclConfigPolicyView& policy :
    ...
    const cocclCompressorPolicyEntry& configured =
        policy.policy->compressor;
    if (configured.name.empty()) continue;
    auto plugin = loadedPlugins.find(configured.name);
    if (plugin == loadedPlugins.end()) {
      valid = false;
      continue;
    }
    const std::string base = std::string(policy.path) + ".config";
    valid = validatePluginConfig(
                plugin->second.compressor, configured.defaultValues,
                cocclCompressorConfigDefault, options, base + ".default") &&
        valid;
    ++validatedBindings;

    if (configured.hasHierarchicalConfig) {
      valid = validatePluginConfig(
                  plugin->second.compressor, configured.hierarchicalValues,
                  cocclCompressorConfigHierarchical, options,
                  base + ".hierarchical") &&
          valid;
      ++validatedBindings;
    }
