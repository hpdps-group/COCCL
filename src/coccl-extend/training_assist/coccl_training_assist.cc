...
constexpr size_t kDetectionInterval = 128;
...
void cocclTrainingAssistRegister(ncclComm_t comm) {
  ...
  std::unique_ptr<cocclTrainingAssistCommState> state(
      new (std::nothrow) cocclTrainingAssistCommState());
  if (state == nullptr) {
    WARN("COCCL training assist state allocation failed for comm %p", comm);
    return;
  }
    ...
    try {
      ...
      if (cocclTrainingEvents.empty()) {
        try {
          ...
          WARN("COCCL training trace reservation failed; using incremental allocation");
        }
      }
    } catch (const std::bad_alloc&) {
      WARN("COCCL training assist registry allocation failed for comm %p", comm);
    }
  }
...
void cocclTrainingAssistObserve(
  ...
  if (!checkedLogicalBytes(args, &logicalBytes)) return;
  ...
  if (state == cocclTrainingAssistComms.end() || allCommsCommittedLocked()) {
    pthread_mutex_unlock(&cocclTrainingAssistLock);
    return;
  }
  ...
  bool reachedLimit = allocationFailed ||
                      cocclTrainingEvents.size() >= maxEvents;
  ...
  bool detectionPoint = cocclTrainingEvents.size() == firstDetectionPoint ||
                        (!cocclTrainingEvents.empty() &&
                         cocclTrainingEvents.size() % kDetectionInterval == 0);
