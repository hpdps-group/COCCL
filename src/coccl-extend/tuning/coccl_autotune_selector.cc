...
// Hierarchical algorithms assume every node contributes the same number of
// ranks so each local and inter-node phase has a uniform shard layout.
bool uniformNodeRanks(ncclComm_t comm) {
  if (comm == nullptr || comm->nNodes <= 0 || comm->localRanks <= 0) {
    return false;
  }
...
// Auto mode must score every eligible policy variant. Forced and heuristic
// paths resolve only the compressor selected by the final decision.
...
ncclResult_t resolveHierarchicalCompressor(
  ...
  NCCLCHECK(cocclCommResolveCompressorPolicy(
      prepared->info.comm, prepared->trainingRole,
      cocclHierarchicalPolicy(prepared->info.operation), &resolved));
  *compressor = std::move(resolved.compressor);
  return ncclSuccess;
}
...
void setCandidateScore(cocclPreparedCall* prepared,
  ...
  // Preserve the public diagnostic fields without duplicating this mapping in
  // each operation-specific scoring formula.
...
void warnForcedFallback(cocclOperation operation) {
  ...
  } else if (operation == cocclOperation::AllReduce) {
...
double messageBytes(const cocclPreparedCall& prepared) {
  ...
  double bytes =
      (double)info.count * (double)ncclTypeSize(info.datatype);
...
ncclResult_t selectCandidate(cocclPreparedCall* prepared) {
  ...
  bool divisibleByRanks =
      comm->nRanks > 0 && info.count % (size_t)comm->nRanks == 0;
  cocclAutotuneEligibility eligibility = {
  ...
  // AllReduce shape failures remove every table entry and retain the previous
  // ncclInvalidArgument behavior.
  if (candidates.count == 0) return ncclInvalidArgument;

  const cocclAutotuneConfig& config = cocclGetConfig().autotune;
  cocclAlgorithmKind requested = configuredAlgorithm(info.operation);
  const bool scoreCandidates =
      requested == cocclAlgorithmNone && config.enabled;
  const cocclCompressorHandle baseCompressor = prepared->compressor;
  cocclCompressorHandle hierarchicalCompressor;

  if (scoreCandidates) {
    // Resolve each policy variant once, snapshot all models under one lock, and
    // then score the fixed candidate set without further runtime queries.
    ...
    cocclSelectionPerformanceModel performance =
        cocclAutotuneSnapshotPerformanceModel(
            baseCompressor,
            needHierarchical ? &hierarchicalCompressor : nullptr,
  ...
  // Commit executable fields only after selection and any required policy
  // resolution succeed; score fields remain diagnostics for attempted models.
...
ncclResult_t cocclSelectAlgorithm(cocclPreparedCall* prepared) {
  ...
  if (prepared->info.operation == cocclOperation::ReduceScatter ||
    ...
    result = selectCandidate(prepared);
  }
