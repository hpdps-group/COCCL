...
ncclResult_t  ncclAllToAllComp(const void* sendbuff, void* recvbuff, size_t sendcount,
  ...
  // NCCLCHECK(ncclCompress(sendbuff, &sendCompbuff, sendcount, datatype, &compSendCount, &compDatatype, comm->nRanks, comm, ncclCommOp_t::AlltoAll, stream));
  NCCLCHECK(ncclCompress(sendbuff, &workspaceBase, sendcount, datatype, &compSendCount, &compDatatype, comm->nRanks, comm->rank, comm, ncclCommOp_t::AlltoAll, stream));
  ...
  NCCLCHECK(ncclDecompress(recvbuff, (char*)recvCompbuff, sendcount, datatype, compSendCount, compDatatype, comm->nRanks, comm, ncclCommOp_t::AlltoAll, stream));
