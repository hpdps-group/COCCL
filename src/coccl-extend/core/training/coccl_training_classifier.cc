...
struct cocclTrainingCommStats {
  ...
  bool stableCollectivePattern = false;
};
...
static void buildCommStats(
  ...
  *stats = {};
  ...
  for (const cocclTrainingTraceEvent& event : events) {
    ...
    stats->calls++;
    if (cocclTrainingIsP2p(event.operation)) {
      stats->p2pCalls++;
      peerCounts[event.peer]++;
      continue;
    }
    if (!cocclTrainingIsCollective(event.operation)) continue;
  ...
  stats->medianBytes = cocclTrainingMedian(collectiveBytes);
  ...
  for (const cocclTrainingIterationRange& iteration : iterations) {
    ...
    if (!pattern.empty()) {
      ...
      bool hasMultiSizeOperation = false;
      for (const auto& operation : sizesByOperation) {
        hasMultiSizeOperation |= operation.second.size() >= 2;
      }
      if (hasMultiSizeOperation) multiSizePatternCounts[pattern]++;
    }
  ...
  stats->callsPerIteration = (double)cocclTrainingMedian(callsByIteration);
  ...
  stats->representativeAgToRsRatio = matchingRatios.empty()
      ? 0.0 : cocclTrainingMedian(matchingRatios);
  stats->stableCollectivePattern = stats->collectiveCalls != 0 &&
  ...
  for (const cocclTrainingIterationRange& iteration : iterations) {
    ...
    for (size_t i = iteration.begin; i < iteration.end && i < events.size(); ++i) {
      ...
      for (size_t j = i + 1; j < iteration.end && j < events.size(); ++j) {
        ...
        if (next.communicatorId == tpComm && cocclTrainingIsCollective(next.operation)) {
          tpCollectives++;
        }
    ...
    for (size_t i = iteration.begin; !tail && i < iteration.end && i < events.size(); ++i) {
      ...
      for (size_t j = i; j > iteration.begin && examinedCollectives < 8;) {
        ...
        if (!cocclTrainingIsCollective(previous.operation)) continue;
        examinedCollectives++;
...
static double ppInteriorSupport(
  ...
  for (const cocclTrainingIterationRange& iteration : iterations) {
    ...
    for (size_t i = iteration.begin; i < iteration.end && i < events.size(); ++i) {
      const cocclTrainingTraceEvent& event = events[i];
      if (ppComms.count(event.communicatorId) != 0 && cocclTrainingIsP2p(event.operation)) {
        ...
        candidateCollectives = 0;
                 ...
                 cocclTrainingIsCollective(event.operation)) {
...
cocclTrainingRole cocclTrainingTopologyRole(
  ...
  if (cocclTrainingIsP2p(operation)) {
    return config.pipelineParallelSize > 1 &&
            communicator.nRanks == config.pipelineParallelSize
        ? cocclTrainingRolePipelineParallel
        : cocclTrainingRoleUnknown;
  }
  if (!cocclTrainingIsCollective(operation)) return cocclTrainingRoleUnknown;

  const bool dataParallel =
      communicator.nRanks == config.dataParallelSize;
  const bool tensorParallel =
      communicator.nRanks == config.tensorParallelSize;
  if (dataParallel == tensorParallel) return cocclTrainingRoleUnknown;
  return dataParallel ? cocclTrainingRoleDataParallel
...
void cocclTrainingClassifyTrace(
  ...
  for (const cocclTrainingTraceComm& communicator : communicators) {
    ...
    cocclTrainingRole topologyRole = cocclTrainingRoleUnknown;
    if (stats.p2pCalls == stats.calls && stats.calls != 0) {
      topologyRole = cocclTrainingTopologyRole(
          stats.descriptor, ncclFuncSend, config);
    } else if (stats.collectiveCalls != 0) {
      topologyRole = cocclTrainingTopologyRole(
          stats.descriptor, ncclFuncAllReduce, config);
    }
    if (topologyRole != cocclTrainingRoleUnknown) {
      if (topologyRole == cocclTrainingRoleDataParallel) evidence.dp = 1.0;
      else if (topologyRole == cocclTrainingRoleTensorParallel) evidence.tp = 1.0;
      else evidence.pp = 1.0;
      continue;
    }

    if (config.pipelineParallelSize > 1 && stats.p2pShare >= 0.70 &&
        stats.peerConcentration >= 0.80) {
    ...
    if (config.dataParallelStrategy == cocclDataParallelStrategy::Ddp) {
      bool ddpBuckets = stats.descriptor.nNodes > 1 &&
                        ...
                        stats.reduceScatterCalls == 0 &&
                        stats.stableCollectivePattern &&
                        stats.callsPerIteration >= 2.0;
      if (ddpBuckets) {
        evidence.dp = stats.patternSupport * calibrationCoverage;
      }
    } else if (config.dataParallelStrategy ==
               cocclDataParallelStrategy::Fsdp) {
      bool fsdpFlow = stats.descriptor.nNodes > 1 &&
                      stats.allGatherCalls != 0 &&
                      stats.reduceScatterCalls != 0 &&
                      stats.stableCollectivePattern;
      if (fsdpFlow) {
    ...
    bool tensorAllReduce =
        !config.sequenceParallel &&
        config.dataParallelStrategy != cocclDataParallelStrategy::Ddp &&
  ...
  // A communicator with direct DP evidence can identify a stable node-local TP
  // flow by relative order. Without such a seed, only the established
  // node-local sparse-DP/dense-TP tail pattern assigns both roles.
  for (const auto& dpEntry : statsById) {
    ...
    for (const auto& tpEntry : statsById) {
      ...
      bool seededDp = evidenceById[dpEntry.first].dp >=
                      kRoleEvidenceThreshold;
      bool sparseNodeLocalTail = dpEntry.second.descriptor.nNodes == 1 &&
          dpEntry.second.stableCollectivePattern &&
          tpEntry.second.stableCollectivePattern &&
          dpEntry.second.callsPerIteration < tpEntry.second.callsPerIteration &&
          dpEntry.second.medianBytes >=
              (size_t)((double)tpEntry.second.medianBytes * 1.25);
      if (!seededDp && !sparseNodeLocalTail) continue;

      if (!seededDp) {
        evidenceById[dpEntry.first].dp = relation;
      }
      evidenceById[dpEntry.first].orderAsDp =
          std::max(evidenceById[dpEntry.first].orderAsDp, relation);

      if (evidenceById[tpEntry.first].dp < kRoleEvidenceThreshold &&
          tpEntry.second.stableCollectivePattern &&
          tpEntry.second.callsPerIteration >= 2.0) {
        evidenceById[tpEntry.first].tp =
            std::max(evidenceById[tpEntry.first].tp, relation);
  ...
  // PP boundaries provide a second way to recognize a stable TP burst when
  // the DP communicator is absent or uses a different schedule.
  for (const auto& entry : statsById) {
    if (entry.second.descriptor.nNodes > 1 ||
        evidenceById[entry.first].pp >= 0.70 ||
        evidenceById[entry.first].dp >= kRoleEvidenceThreshold ||
        !entry.second.stableCollectivePattern ||
  ...
  // With a known DP communicator, a repeated node-local flow with at least two
  // collectives per iteration is a TP candidate even when scheduling obscures
  // a strict pairwise relation.
