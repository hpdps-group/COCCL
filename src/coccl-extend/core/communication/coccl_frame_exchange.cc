  ...
  for (int peer = 0; peer < nRanks; ++peer) {
    for (size_t frame = 0; frame < framesPerPeer; ++frame) {
      ...
      exchanges[(*exchangeCount)++] = {
          ...
          (size_t)recv.payloadBytes, frameStrideBytes, comm, stream};
    }
  }
  return ncclSuccess;
}
...
ncclResult_t cocclCommitFrameExchange(
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    if (exchange.comm == nullptr || exchange.peer < 0 ||
        exchange.peer >= exchange.comm->nRanks ||
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    if (exchange.recvBytes != 0) {
      ...
      info.comm = exchange.comm;
      info.stream = exchange.stream;
