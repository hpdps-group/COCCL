...
StageResource communicationResource(
    ncclComm_t owner, const cocclPipelineStage& stage) {
  const int policy = cocclPipelineStageCtaPolicy(owner, stage);
  return (policy & NCCL_CTA_POLICY_ZERO) != 0
      ? StageResource::IndependentCommunication
      : StageResource::NcclSm;
}
...
StageGraph buildStageGraph(const cocclPipelineSpec* spec) {
  ...
  for (int index = 0; index < spec->stageCount; ++index) {
    ...
    switch (pipelineStage.kind) {
      ...
      case cocclPipelineStageAllGather:
      case cocclPipelineStageReduceScatter: {
        stage.resource = communicationResource(spec->ownerComm, pipelineStage);
