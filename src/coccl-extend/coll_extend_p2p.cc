...
// helpers remain useful for ring/two-shot variants.
...
ncclResult_t ncclReduceScatterRing(const void* sendbuff, void* recvbuff, size_t recvcount,
    ...
    // Walk the ring backward so each rank forwards one chunk while reducing the
...
ncclResult_t ncclAllReduceRing(const void* sendbuff, void* recvbuff, size_t count,
  ...
  // Ring AllReduce is composed as ReduceScatter followed by AllGather, matching
  // the compressed ring variant in coccl_allreduce.cc.
